...
require 'database.php';
    ...
    if(!empty($_GET['id']))
    {
        $id = checkInput($_GET ['id']);  // nettoyer les donnée et valide entré utilisateur
    }
...
if(!empty($_POST))
{
    ...
    // Si tout est valide , insertion des données dans labase de donéess
        ...
        if(($isSuccess && $isImageUpdate && $isUploadSuccess) || ($isSuccess && !$isUploadSuccess))
        {
            $db = Database::connect();
            if($isImageUpdate)
            {
...
<h1 class="text-logo"><span class="glyphicon glyphicon-cutlery"></span> Burger-Paris <span class="glyphicon glyphicon-cutlery"></span></h1>
    ...
    <div class="col-sm-6 ">
            ...
            <h2><strong> Modifier un Items  </strong></h2>
            <br>
            <form class="form" role="form" action="update.php?id=" method="post" enctype="multipart/form-data">
    ...
    // selection base de donnée
                                ...
                                { 
                                    $selected = ($row['id'] == $category) ? 'selected' : ''; // Vérifie si l'option doit être sélectionnée
